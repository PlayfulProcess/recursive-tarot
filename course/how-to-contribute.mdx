...
- **Easy route — in the app.** Open any card on the [card browser](../viewers/cards.html?src=../tarot/visconti-sforza-tarot/grammar.json) and use the **✏️ Edit in recursive.eco** button. It opens the deck in recursive.eco's Create editor — make your change and save; if it isn't your own deck, the edit is proposed to the owner for review. No git, no raw JSON.
...
Pick a deck and open it. The **recursive.eco** logo opens the live grammar in the app — read it, then use **Edit** to propose a fix (Rung 1). The **GitHub** logo opens that deck's `grammar.json` straight in GitHub's web editor (Rung 2). Two doors to the same deck — use whichever feels easier.
...
> **Look for the two purple buttons.** Anywhere you're reading a deck in this site — the [card browser](../viewers/cards.html?src=../tarot/visconti-sforza-tarot/grammar.json), the [Caster](../viewers/caster.html) — every card carries two small purple recursive.eco buttons: <span style="white-space:nowrap">▶ **Cast in recursive.eco ↗**</span> (opens the deck in the Journal, ready to draw — for reading, not fixing) and <span style="white-space:nowrap">✏️ **Edit in recursive.eco ↗**</span> (opens the deck in Create — this is the one you want here). The same two buttons, the same wording, mark every card across the whole site — so you learn them once and recognise them everywhere. (The Edit button only appears on a real deck's own page like this one — not on the "All Decks" meta view, whose cards are pointers into other decks and aren't directly editable.)
3. Use **Edit in recursive.eco**, correct the text statement, and **save** — it comes to the maintainer (me) to review and merge back into the public deck (or saves straight away if it's already your own deck). Your name rides along in the version history.
...
If you connect claude.ai's Code tab to GitHub instead of copy-pasting, there's a quiet safety net worth knowing: every session automatically works on its own branch, so your versions are protected and no work gets lost — even if you close the tab mid-edit. (What a branch is, and why that guarantee holds, is explained gently in [Working with Claude Desktop](course-viewer.html?course=working-with-claude-desktop).)
...
This rung has its own deep dive — [Working with Claude Desktop](course-viewer.html?course=working-with-claude-desktop) — which starts from zero: what GitHub and git actually are, the session habits that guarantee no work is lost, how to file a good pull request, and how to keep private notes private while contributing to the public commons.
