...
signed-out; signed-in-only paths are flagged for PlayfulProcess's browser test.*
...
PlayfulProcess's brief: *"I don't want a view that is complex and does not generate insights… my idea
...
- Signed-in-only paths needing PlayfulProcess's test: My-spreads dropdown (`/api/spreads`), model-quality
  hint banner.
