...
Method: phases sized in sessions, each with a gate; PlayfulProcess tests thoroughly at the end;
ONE merge to main. Local browser verification before any push (standing rule).*

**North star for the Explorer (PlayfulProcess, Aug 7):** *"something scholars or artists could use to
...
Gate: PlayfulProcess hovers Home/Views/Courses/Play on the live-local site; then push (Pages build is
...
every 3-of-suit — the "grouping most number cards together" PlayfulProcess saw); `arcana` splits into
...
Gate: PlayfulProcess pivots for 5 minutes and reports whether it *feels* like a comparison instrument.
...
license nobody asked the knowledge-holders about. Options for PlayfulProcess to pick:
