...
- [x] **EX-SECTIONS** — DONE: the Sections mode now (1) renders each card's chosen section in FULL (removed the 260-char truncation + the 8-line clamp), (2) is resizable — every section box has `resize:both` (drag the corner) and the toolbar size slider bulk-sizes them via `--esec-w`/`--esec-h`, and (3) fixes the dark-theme bug: the boxes were dark cards (`rgba(12,9,22,.55)` bg, near-white text) on a light-only site — now light (`--chip` bg, `--ink` text) with the section-picker text darkened too. The `§ Sections` dropdown works like the cards viewer's. Verified in preview (light boxes, full text, resize handle). Also fixed a stray card name ("0 — The Fool test" → "0 — The Fool" in Visconti-Sforza). (prior #2)
