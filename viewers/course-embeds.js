                  ...
                  if(synEls.length||detEls.length||sdetEls.length){
                    ...
                    if(synEls.length){ let html='';
                      for(const tk of ORDER){ if(!syn||!syn[tk])continue;
                        ...
                        const cells=entries.map(c=>`<a class="c" href="../viewers/cards.html?src=../tarot/${c.slug}/grammar.json&item=${encodeURIComponent(c.id)}"><img loading="lazy" src="${esc(c.img)}"><div>${esc(c.name.slice(0,14))}</div></a>`).join('');
